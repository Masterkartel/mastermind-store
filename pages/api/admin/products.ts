...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === "POST") {
    ...
    const { name, price, stock, product_code, sku, img } = req.body || {};
  ...
  if (req.method === "PUT") {
    ...
    db.products[idx].price = Number((db.products[idx] as any).price) || 0;
    db.products[idx].retail_price = Number((db.products[idx] as any).retail_price ?? (db.products[idx] as any).price) || 0;
    db.products[idx].stock = Number((db.products[idx] as any).stock) || 0;
    await writeStore(db);
