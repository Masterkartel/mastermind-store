...
// --- Make TS aware of Paystack on window ---
declare global {
  interface Window {
    PaystackPop?: any;
  }
...
export default function Home() {
  ...
  // ---- Load Paystack inline script (browser only) ----
  useEffect(() => {
    ...
    return () => {
      // keep it loaded between navigations; no removal
    };
  }, []);
  ...
  const startPaystack = () => {
    ...
    if (!pubKey) {
      alert("Paystack not configured. Please set NEXT_PUBLIC_PAYSTACK_PUBLIC_KEY.");
      return;
    }
    ...
    // Paystack amount expects Kobo/Cents. For KES, Paystack still wants smallest unit,
    // so multiply by 100.
    const amountKobo = Math.round(cartTotal * 100);

    const reference = `MM-${Date.now()}`;
    const metaItems = cartLines.map((l) => ({
      id: l.product.id,
      name: l.product.name,
      qty: l.qty,
    ...
    const handler = window.PaystackPop.setup({
      key: pubKey,
      email: "guest@mastermindelectricals.com", // we are not collecting user email yet
      amount: amountKobo,
      currency: "KES",
      reference,
      metadata: {
        custom_fields: [
          {
            display_name: "Cart",
            variable_name: "cart",
            value: JSON.stringify(metaItems),
          },
        ],
      },
      callback: function (_resp: any) {
        // Success — clear cart and say thanks
        clear();
        setShowCart(false);
        alert("Payment received. Thank you! We’ll confirm your order shortly.");
      },
      onClose: function () {
  ...
  return (
    <div style={{ fontFamily: "Inter, ui-sans-serif", background: "#fafafa" }}>
      ...
      <style jsx>{`
        ...
        .cylImg--tight { margin-bottom:-8px; } /* bring 6KG closer to its button */
        ...
        .btn--paystack { background:#0aa83f; color:#fff; border:none; padding:12px 16px; border-radius:12px; } /* Paystack green */
